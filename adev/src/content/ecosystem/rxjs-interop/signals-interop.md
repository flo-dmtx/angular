...
In all other ways, `rxResource` behaves like and provides the same APIs as `resource` for specifying parameters, reading values, checking loading state, and examining errors. This includes the [`loadStrategy` option](/guide/signals/resource#lazy-resources), whose lazy strategies defer the subscription until something actually tracks the resource.
